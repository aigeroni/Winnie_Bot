...
export class Goal extends BaseModel {
  ...
  /**
    ...
    */
  @Column({ name: 'updated_at', transformer: dateTransformer })
  updatedAt?: DateTime
   ...
   */
  @Column({ name: 'canceled_at', transformer: dateTransformer })
  canceledAt?: DateTime
   ...
   */
  @Column({ name: 'completed_at', transformer: dateTransformer })
  ...
  timeRemaining (): Duration {
    ...
    const timeRemaining = Interval.fromDateTimes(this.createdAt, this.endDate()).toDuration()
  ...
  async print (locale: string): Promise<string> {
    ...
    const timeString = await I18n.translate(locale, 'goals:timeRemaining', {
      months: await I18n.translate(locale, 'goals:durationsWithCount.months', { count: timeRemaining.months }),
      days: await I18n.translate(locale, 'goals:durationsWithCount.days', { count: timeRemaining.days }),
      hours: await I18n.translate(locale, 'goals:durationsWithCount.hours', { count: timeRemaining.hours }),
      minutes: await I18n.translate(locale, 'goals:durationsWithCount.minutes', { count: timeRemaining.minutes })
    })
