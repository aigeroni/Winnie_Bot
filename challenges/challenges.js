  ...
  joinChallenge(msg, suffix) {
    ...
    if (isNaN(challengeID)) {
      ...
      if (challengelist.challengeList[challengeID].hidden && client
        ...
        msg.channel.send(msg.author + ', you do not have permission to join'
         + ' this challenge.');
      } else {
        if (msg.author.id in challengelist.challengeList[challengeID]
            .joinedUsers) {
          msg.channel.send(msg.author + ', you already have notifications'
            + ' enabled for this challenge.');
        } else {
          challengelist.challengeList[challengeID].joinedUsers[msg.author.id]
                = {'countData': undefined,
              'countType': undefined, 'channelID': msg.channel.id};
          const pushID = msg.channel.id;
          const searchIndex = challengelist.challengeList[challengeID]
          ...
          try {
            conn.collection('challengeDB').update(
                ...
                {$set: {'joinedUsers': challengelist.challengeList[challengeID]
                    .joinedUsers}},
                {upsert: true}
            );
          } catch (e) {
  ...
  leaveChallenge(msg, suffix) {
    ...
    if (isNaN(challengeID)) {
      ...
      if (msg.author.id in challengelist.challengeList[challengeID]
        ...
        msg.channel.send(msg.author + ', you have not yet joined this'
          + ' challenge.');
      }
    } else {
      msg.channel.send('Challenge ' + challengeID + ' does not exist!');
  ...
  */
  startSprint(msg, suffix) {
    ...
    if (!Number.isInteger(Number(words))) {
      ...
      msg.channel.send('Sprints cannot start more than 30 minutes in the'
        + ' future.');
    } else if (timeout > 60) {
      ...
      try {
        ...
        challengelist.challengeList[this.timerID] = new Sprint(this.timerID,
            creatorID, sprintName, startTime, start, words, timeout,
            msg.channel.id, this.crossServerStatus[msg.guild.id], {});
  ...
  startWar(msg, suffix) {
    ...
    if (isNaN(start)) {
      ...
      try {
        ...
        const startTime = new Date().getTime();
        challengelist.challengeList[this.timerID] = new War(this.timerID,
            creatorID, warName, startTime, start, duration, msg.channel.id,
  ...
  startChainWar(msg, suffix) {
    ...
    if (isNaN(chainWarCount)) {
      ...
      try {
        ...
        const startTime = new Date().getTime();
        challengelist.challengeList[this.timerID] = new ChainWar(this.timerID,
            creatorID, warName, startTime, 1, chainWarCount, timeBetween,
            duration, msg.channel.id, this.crossServerStatus[msg.guild.id], {});
        conn.collection('timer').update(
            {data: this.timerID},
            {data: (this.timerID+1)},
            {upsert: true}
  ...
  */
  stopChallenge(msg, suffix) {
    ...
    if (isNaN(challengeID) || challengeID < 1) {
      ...
      if (!(challengelist.challengeList[challengeID].hidden && challengelist
        ...
        if (challengelist.challengeList[challengeID].creator == msg.author.id) {
          ...
          for (let i = 0; i < challengelist.challengeList[challengeID]
              .hookedChannels.length; i++) {
            client.channels.get(challengelist.challengeList[challengeID]
                .hookedChannels[i]).send(stopName
        ...
        msg.channel.send(msg.author + ', you do not have permission to end'
          + ' this challenge.');
      }
    } else {
      msg.channel.send('Challenge ' + challengeID + ' does not exist!');
  ...
  listChallenges(client, msg) {
    ...
    for (const i in challengelist.challengeList) {
      if (object.hasOwnProperty(variable)) {
        const parentChannel = client.channels.get(challengelist.challengeList[i]
            .channelID);
        const parentGuild = parentChannel.guild;
        // check whether a challenge is hidden
        if (!(challengelist.challengeList[i].hidden && parentGuild.id
            != msg.guild.id)) {
          nonHiddenTotal += 1;
          ...
          switch (challengelist.challengeList[i].state) {
            case 0:
              if ((challengelist.challengeList[i].cStart % 60) < 10) {
                timeout = '0' + (challengelist.challengeList[i].cStart % 60)
                    .toString();
              } else {
                timeout = challengelist.challengeList[i].cStart % 60;
              }
              timerInfo += i + ': ' + challengelist.challengeList[i].displayName
                + ' (';
              ...
              timerInfo += challengelist.challengeList[i].duration
                + ' minutes, starts in ' + Math.floor(challengelist
                  .challengeList[i].cStart/ 60) + ':' + timeout + '), '
                  + parentGuildName + '\n';
              break;
            case 1:
              if ((challengelist.challengeList[i].cDur % 60) < 10) {
                timeout = '0' + (challengelist.challengeList[i].cDur % 60)
                    .toString();
              } else {
              ...
              timerInfo += challengelist.challengeList[i].duration
                + ' minutes, ' + Math.floor(challengelist.challengeList[i].cDur
                / 60) + ':' + timeout + ' remaining), ' + parentGuildName
                + '\n';
              break;
            case 2:
            case 3:
              timerInfo += i + ': ' + challengelist.challengeList[i]
                  .displayName + ' (';
              if (challengelist.challengeList[i].type == 'sprint') {
                timerInfo += challengelist.challengeList[i].goal + ' words, ';
              }
            ...
            if (Number.isInteger(Number(wordsWritten))) {
              for (const user in challengelist.challengeList[challengeID]
                ...
                if (user == msg.author.id) {
                  if (!(challengelist.challengeList[challengeID]
              ...
              raptorCheck = false;
              msg.channel.send(msg.author + ', I need a whole number to include'
                + ' in the summary!');
            }
          } else {
            raptorCheck = false;
    ...
    if (suffix == '') {
      ...
      if (msg.member.permissions.has('ADMINISTRATOR')) {
        if (suffix == 'on' || suffix == 'off') {
          ...
          msg.channel.send(msg.author + ', use **on|off** to toggle'
            + ' cross-server challenges.');
        }
      } else {
        msg.channel.send('Only server administrators are permitted to configure'
          + ' challenges.');
      }
    }
  }
