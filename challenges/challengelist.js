      ...
      if (this.challengeList[challengeID].state >= 2) {
        ...
        for (const user in this.challengeList[challengeID].joinedUsers) {
          if (Number.isInteger(Number(this.challengeList[challengeID]
              .joinedUsers[user].countData)) && this.challengeList[challengeID]
              .joinedUsers[user].countType != undefined) {
            const homeServer = client.channels.get(this
            ...
            if (homeServer == summaryServer.id) {
              userTotal += client.users.get(user) + ': **'
                ...
                + '** ';
              if (this.challengeList[challengeID].joinedUsers[user].countData
                == 1) {
                userTotal += this.challengeList[challengeID].joinedUsers[user]
                    .countType.slice(0, -1)+ '\n';
              } else {
        ...
        let firstType = true;
        let summaryData = '***Statistics for ' + this.challengeList[challengeID]
            .displayName + ':***\n\n' + userTotal + summaryServer.name
            + ' Total:';
        ...
        let crossData = false;
        for (let i = 0; i < this.challengeList[challengeID]
          ...
          if (currentServer.id != channel.guild.id) {
            ...
            if (totalMinutes[currentServer] > 0) {
              ...
              if (totalMinutes[currentServer] == 1) {
                summaryData += '** minute';
              } else {
                summaryData += '** minutes';
              }
        ...
        if (crossData) {
          if (firstType) {
            msgToSend += '***Statistics for ' + this.challengeList[challengeID]
                .displayName + ':***\n';
          }
