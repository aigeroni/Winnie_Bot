...
class Tools {
  ...
  calcTarget(msg, suffix) {
    ...
    if (!Number.isInteger(Number(time))) {
      ...
      if (base === null) {
        ...
        const goalPerMinute = ((Math.ceil(Math.random() * 12)
                    + base));
  ...
  */
  getPrompt(msg) {
    const choiceID = (Math.floor(Math.random() * prompts
        .PROMPT_LIST.length));
    ...
    for (let i = 0; i < faces.length; i++) {
      if (Number.isInteger(Number(faces[i]))) { // Single number
        ...
        if (!Number.isInteger(Number(rpgRoll[0])) ||
          ...
          diceString = 'Error: Both values in an RPG-style roll must be'
            + ' integers.';
          diceSum = 0;
          break;
        } else {
        ...
        if (!Number.isInteger(Number(rangeRoll[0])) ||
          ...
          if (Number(rangeRoll[0]) < Number(rangeRoll[1])) {
            const roll = (Math.floor(Math.random() * (1 + Number(rangeRoll[1])
              - Number(rangeRoll[0])) + Number(rangeRoll[0])));
            diceString += roll;
            diceSum += roll;
